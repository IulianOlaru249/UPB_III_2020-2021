...
void Laborator3_Vis2D::Init()
{
	...
	logicSpace2.x = 0;		// logic x
	logicSpace2.y = 0;		// logic y
	logicSpace2.width = 4;	// logic width
	logicSpace2.height = 4;	// logic height
...
void Laborator3_Vis2D::Update(float deltaTimeSeconds)
{
	...
	viewSpace2 = ViewportSpace(resolution.x / 2, 0, resolution.x / 2, resolution.y);
	SetViewportArea(viewSpace2, glm::vec3(0.5f), true);

	// Compute uniform 2D visualization matrix
	visMatrix = glm::mat3(1);
	visMatrix *= VisualizationTransf2DUnif(logicSpace2, viewSpace2);
	DrawScene(visMatrix);
}
